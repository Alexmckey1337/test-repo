...
class SummitSearchUserDeviceView(GenericAPIView):
    ...
    def get_queryset(self, *args, **kwargs):
        return SummitAnket.objects.filter(summit__id=kwargs.get('pk'), device_code=kwargs.get('device_id'))
...
class SummitStatisticsView(SummitProfileListView):
    ...
    def annotate_queryset(self, qs):
        subqs = SummitAttend.objects.filter(date=self.filter_date, anket=OuterRef('pk')).annotate(
            first_time=Coalesce(
                ToChar(F('created_at'), function='to_char', time_format='YYYY-MM-DD HH24:MI:SS+0300',
