...
def can_see_summit(user, summit_id):
    ...
    """
    return user.is_summit_consultant_or_high(summit_id) or user.is_staff or user.has_operator_perm
...
def can_see_any_summit(user):
    ...
    """
    return user.is_any_summit_consultant_or_high() or user.is_staff or user.has_operator_perm
...
def can_see_summit_type(user, summit_type):
    ...
    """
    return user.is_summit_type_consultant_or_high(summit_type) or user.is_staff or user.has_operator_perm


def can_see_any_summit_type(user):
    return user.is_any_summit_type_consultant_or_high() or user.is_staff or user.has_operator_perm
...
def can_edit_summit_block(current_user, user):
    ...
    """
    return True or user.is_staff or user.has_operator_perm
...
def can_see_any_summit_ticket(user):
    return user.is_any_summit_supervisor_or_high() or user.is_staff or user.has_operator_perm


def can_see_summit_ticket(user, summit):
    return user.is_summit_supervisor_or_high(summit) or user.is_staff or user.has_operator_perm
...
def can_see_summit_profiles(user, summit):
    return user.is_summit_consultant_or_high(summit) or user.is_staff or user.has_operator_perm
