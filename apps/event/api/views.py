...
class MeetingViewSet(ModelViewSet, EventUserTreeMixin):
    ...
    def get_queryset(self):
        if self.action == 'list':
            ...
            quseyset = quseyset.prefetch_related('attends').annotate_owner_name().annotate(
            ...
            return quseyset.order_by('-status', '-date')

        return self.queryset.for_user(self.request.user).order_by('-status', '-date')
