...
def can_see_churches(user):
    ...
    return (
        ...
        user.__class__.get_tree(user).exclude(church__isnull=True).exists() or user.is_staff or user.has_operator_perm
    )
...
def can_see_church(user, church):
    ...
    """
    return church.pastor.is_descendant_of(user) or church.pastor == user or user.is_staff or user.has_operator_perm
...
def can_create_church(user):
    ...
    """
    return user.is_main_bishop_or_high or user.is_staff or user.has_operator_perm
...
def can_add_user_to_church(current_user, user, church):
    ...
    """
    return user.is_descendant_of(current_user) or current_user.is_staff or user.has_operator_perm
...
def can_export_churches(user):
    ...
    """
    return user.__class__.get_tree(user).exclude(church__isnull=True).exists() or user.is_staff or user.has_operator_perm
...
def can_see_home_groups(user):
    ...
    return (
        user.is_main_bishop_or_high or
        user.__class__.get_tree(user).exclude(home_group__isnull=True).exists() or user.is_staff or user.has_operator_perm
    )
...
def can_see_home_group(user, home_group):
    ...
    """
    return home_group.leader.is_descendant_of(user) or user.is_staff or user.has_operator_perm
...
def can_edit_home_group(user, home_group):
    ...
    """
    return user.is_bishop_or_high or user.is_staff or user.has_operator_perm
...
def can_add_user_to_home_group(current_user, user, home_group):
    ...
    """
    return user.is_descendant_of(current_user) or user.is_staff or user.has_operator_perm
