...
def can_see_churches(user):
    ...
    return (
        ...
        user.get_descendants(include_self=True).exclude(church__isnull=False).exists()
    )
...
def can_see_church(user, church):
    ...
    """
    return church.pastor.is_descendant_of(user)
...
def can_create_church(user):
    ...
    """
    return user.is_main_bishop_or_high
...
def can_add_user_to_church(current_user, user, church):
    ...
    """
    return user.is_descendant_of(current_user)
...
def can_see_home_groups(user):
    ...
    return (
        user.is_main_bishop_or_high or
        user.get_descendants(include_self=True).exclude(home_group__isnull=False).exists()
    )
...
def can_see_home_group(user, home_group):
    ...
    """
    return home_group.leader.is_descendant_of(user)
