...
export function btnDeals(canChooseCurr = false) {
	$('table').on('click', '.pay', function () {
		...
		options = {diff, currencyName, currencyID, total: total_sum};
		canChooseCurr ? sumCurChange(options) : sumChange(options);
...
function sumCurChange({diff, currencyName, currencyID, total}) {
	let paymentRate = $('#new_payment_rate'),
		paymentSum = $('#new_payment_sum'),
		paymentCurr = $('#new_payment_currency');
	//Unbind all events
	paymentRate.unbind('keyup');
	paymentSum.unbind('keyup');
	paymentCurr.unbind('change');
	//First state
	$('#close_sum').text(`${convertNum(diff, ',')} ${currencyName}`);
	paymentCurr.val(currencyID);
	paymentRate.val('1,0').prop('readonly', true);
	paymentSum.val(convertNum(diff, ','));
	$('#user_payment').text(`${convertNum((+diff + +total).toFixed(2), ',')} ${currencyName}`);
	//Events
	paymentCurr.on('change', function () {
		let rate,
			sum = convertNum(paymentSum.val(), '.'),
			result;
		($(this).val() === currencyID) ?
			paymentRate.val('1,0').prop('readonly', true)
			:
			paymentRate.prop('readonly', false);
		rate = convertNum(paymentRate.val(), '.');
		result = parseFloat((sum / rate).toFixed(2));
		$('#user_payment').text(`${convertNum((+result + +total), ',')} ${currencyName}`);
	});

	paymentRate.on('keyup', _.debounce(function () {
		let rate = convertNum($(this).val(), '.'),
			sum = parseFloat((diff * rate).toFixed(2));
			paymentSum.val(convertNum(sum, ','));
			$('#user_payment').text(`${convertNum((+diff + +total), ',')} ${currencyName}`);
	}, 500));

	paymentSum.on('keyup', _.debounce(function () {
		let rate = convertNum(paymentRate.val(), '.'),
			sum = convertNum($(this).val(), '.'),
			result = parseFloat((sum / rate).toFixed(2));
		$('#user_payment').text(`${convertNum((+result + +total), ',')} ${currencyName}`);
	}, 500));
}
