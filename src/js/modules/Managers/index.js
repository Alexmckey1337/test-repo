...
function getHomeGroupUsers(config) {
    return new Promise(function (resolve, reject) {
        ajaxRequest(URLS.manager.table(), config, function (data) {
            if (data) {
                resolve(data);
            } else {
                reject("Ошибка")
...
export function createHomeGroupsTable(config = {}) {
    ...
    getData(URLS.manager.table(), config).then(function (data) {
        let count = data.count;
        let page = config['page'] || 1;
        let pages = Math.ceil(count / CONFIG.pagination_count);
        let showCount = (count < CONFIG.pagination_count) ? count : data.results.length;
